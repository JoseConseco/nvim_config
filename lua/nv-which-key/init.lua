...
wk.add({
    ...
    { "<leader>?", ":Grepper -cword -noprompt -tool ag<cr>", desc = "Word to clist (Grepper)" },
...
wk.add({
  ...
  -- ['<leader>cc'] = {':TSContextToggle<CR>',             'Context toggle'}, --from treesitter-context plug
    ...
    { "<leader>cd", function() vim.diagnostic.open_float() end, desc = "Diagnostic Float" },
...
wk.add({
    ...
    { "<leader>da", function()
		...
		end, desc = "Attach (localhost, 5678)" },
    ...
    { "<leader>dh", function() require'hydra'.spawn['dap_hydra']() end, desc = "Dap Hydra" },
...
wk.add({
    ...
    { "<leader>gc", compare_to_clipboard, desc = "Diff with clipboard" },
...
wk.add({
    ...
    { "<leader>oc", function()
		...
		end, desc = "Codi Start (multi lang REPL)" },
...
wk.add({
    ...
    { "<leader>qq", ":TSContextDisable<cr>|:call v:lua.Save_current_session()<CR>|:confirm qa", desc = "Quit Confirm (qa)" },
    { "<leader>qs", ":bufdo update | q!<CR>", desc = "Quit Save all (wqa!)" },
})
wk.add({  -- second one for visual mode
    { "<leader>q", group = "Quit", mode = "v" }, { "<leader>qq", ":<ESC>|:TSContextDisable<cr>|:call v:lua.Save_current_session()<CR>|:confirm qa<cr>", desc = "Quit Confirm (qa)", mode = "v" },
})
